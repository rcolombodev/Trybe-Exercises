...
const verifyAnswers = (rightAnswers, studentAnswers, points) => {
  let count = 0;
    for (let index in rightAnswers) {
      const pointscount = points(rightAnswers[index], studentAnswers[index]);
      count += pointscount
    }
    return `Total de pontos: ${count}`;
}

const verifyResult = (rightAnswer, studentAnswer) => {
...
console.log(verifyAnswers(RIGHT_ANSWERS, STUDENT_ANSWERS, verifyResult));